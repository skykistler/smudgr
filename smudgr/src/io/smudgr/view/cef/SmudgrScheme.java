...
/**
 * The example for the second scheme with domain handling is a more complex example and is taken from the parent project CEF. Please see CEF: "cefclient/scheme_test.cpp" for futher details
 */
public class SmudgrScheme extends CefResourceHandlerAdapter {
	...
	public synchronized boolean processRequest(CefRequest request, CefCallback callback) {
		...
		if (url.indexOf("handler.html") != -1) {
			...
			String html;
			html = "<html><head><title>Client Scheme Handler</title></head>" + "<body bgcolor=\"white\">" + "This contents of this page page are served by the " + "ClientSchemeHandler class handling the client:// protocol." + "<br/>You should see an image:" + "<br/><img src=\"client://tests/logo.png\"><pre>";

			// Output a string representation of the request
			html += request.toString();

			html += "</pre><br/>Try the test form:" + "<form method=\"POST\" action=\"handler.html\">" + "<input type=\"text\" name=\"field1\">" + "<input type=\"text\" name=\"field2\">" + "<input type=\"submit\">" + "</form></body></html>";
